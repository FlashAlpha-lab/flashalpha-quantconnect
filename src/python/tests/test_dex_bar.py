...
from flashalpha_quantconnect import DexBar, config
...
def test_dex_bar_fields_match_rest_response():
    ...
    raw = sdk.dex(symbol=TEST_TICKER, at=TEST_DATE)
    ...
    assert bar.NetDex == raw.get("net_dex")
    raw_strikes = raw.get("strikes") or []
    bar_strikes = bar.Strikes or []
    assert len(bar_strikes) == len(raw_strikes)


def test_add_flashalpha_dex_sugar_exists():
    """Compile-time test: the sugar is importable and points to the right bar class."""
    from flashalpha_quantconnect import add_flashalpha_dex
    from flashalpha_quantconnect.data.exposure import DexBar as DirectDexBar
    assert add_flashalpha_dex.__module__ == "flashalpha_quantconnect.extensions"
    assert DirectDexBar is not None
